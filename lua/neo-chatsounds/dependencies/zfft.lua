...
local function butterfly2(iRe, iIm, oidx, fstride, twRe, twIm, isInverse, m, p)
    ...
    repeat
        ...
        i2 = i2 + 1
        m = m - 1
    until m == 0 -- LuaJIT Numeric Performance Guide recommends not doing manual common subexpression elimination... -- However, we can"t do that here, because this backs up values into the locals.
end
...
local function butterfly4(iRe, iIm, oidx, fstride, twRe, twIm, isInverse, m, p)
    ...
    if not isInverse then
        repeat
            ...
            i = i + 1
            k = k - 1
        until k == 0 -- part dependent on isInverse -- //
    else
        repeat
            scratchRe[0] = iRe[i + m] * twRe[tw1] - iIm[i + m] * twIm[tw1]
            scratchIm[0] = iRe[i + m] * twIm[tw1] + iIm[i + m] * twRe[tw1]
...
local function butterfly3(iRe, iIm, oidx, fstride, twRe, twIm, isInverse, m, p)
    ...
    local m2 = m * 2
    local tw1, tw2 = 1, 1
    local _, epi3Im = twRe[fstride * m], twIm[fstride * m]
    local i = oidx
    local scratchRe, scratchIm = {}, {}
    repeat
...
local function fft(inputRe, inputIm)
    assert(type(inputRe) == "table", "A lua table needs to be passed as the first parameter.")
    -- Get array size
    local n = #inputRe
    ...
    if not inputIm then
        ...
        assert(type(inputIm) == "table", "A lua table needs to be passed as the second parameter.")
        assert(#inputRe == #inputIm, "Length mismatch between first and second parameters.")
    end
    ...
    local inputStride = 1
    -- Guts
    work(inputRe, inputIm, outputRe, outputIm, outputIndex, factorCurrent, factors, factorIndex, twiddlesRe, twiddlesIm, factorStride, inputStride, false)
    ----
...
local function ifft(inputRe, inputIm)
    ...
    local inputStride = 1
    -- Guts
    work(inputRe, inputIm, outputRe, outputIm, outputIndex, factorCurrent, factors, factorIndex, twiddlesRe, twiddlesIm, factorStride, inputStride, true)
    ----
